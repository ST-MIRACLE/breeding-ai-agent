...
VISION_PROMPT_TEMPLATE = """你是一位番茄育种科研数据处理专家。请仔细查看这张实验数据扫描图片，逐行逐列精准提取表格中的所有数据。
...
2. 【精准读数】数值必须与图片完全一致，小数点、位数不得改动
3. 【保留原始格式】多次测量值分开保留，不要合并计算均值
4. 【空值处理】图片中空白或看不清的单元格填写"未提及"
5. 【表头保留】列名与图片表头一致

【输出格式】
输出标准JSON数组，每个元素是一行数据，键为列名。只输出JSON，不要其他文字。
"""
